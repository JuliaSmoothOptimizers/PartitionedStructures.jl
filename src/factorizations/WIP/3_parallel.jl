...
using ..M_abstract_element_struct,
  ..M_abstract_part_struct, ..M_elt_mat, ..M_part_mat, ..ModElemental_pm, ..ModElemental_em
...
function third_parallel!(
  epm_A::Elemental_pm{T},
  epv_b::Elemental_pv{T},
  epv_x::Elemental_pv{T},
  res::Vector{T};
  etol::Float64 = 1e-6,
  max_iter::Int = 5 * get_n(epm_A),
) where {T}
...
function subproblem3!(
  ...
  every_indices =
    mapreduce((x -> x), ((indice, indicei) -> unique(vcat(indice, indicei))), _indices) # Les concatène sans répétition
  ...
  # _bary = mean(_x)
  # xk = _bary
  vector_bool[index] == false ? xk = 1.0 : xk = res[index]
