...
select_var(crl_var::Vector{Int}, not_treated::Vector{Bool}) =
  filter!(var -> not_treated[var], crl_var)

actualise_not_added!(front::Vector{Int}, not_added::Vector{Bool}) =
  map(i -> not_added[i] = false, front)
actualise_front!(front::Vector{Int}, not_treated::Vector{Bool}) =
  filter!(var -> not_treated[var], front)
