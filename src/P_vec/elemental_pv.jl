...
"""
    Elemental_pv{T,S,Z} <: Part_v{T}

Represent an elemental partitioned-vector.
"""
mutable struct Elemental_pv{T,S,Z} <: Part_v{T}
  N::Int
  n::Int
  eev_set::Vector{Elemental_elt_vec{T,S,Z}}
  v::S
  component_list::Vector{Z}
  permutation::Z # n-size vector
  gpu::Bool
end

default_permutation(n, gpu) = (gpu ? CuVector : Vector)([1:n;])

function Elemental_pv{T,S,Z}(
  N::Int,
  n::Int,
  eev_set::Vector{Elemental_elt_vec{T,S,Z}},
  v::S;
  gpu=false,
  perm=default_permutation(n, gpu),
) where {T,S,Z}
  component_list = map(i -> Vector{Int}(undef, 0), [1:n;])  
  epv = Elemental_pv{T,S,Z}(N, n, eev_set, v, component_list, perm, gpu)
  initialize_component_list!(epv)
  return epv
end

"""
    eev_set = get_eev_set(epv::Elemental_pv{T,S,Z}) where T

Return either the vector of every elemental element-vector of the $(_epv) or the `i`-th elemental element-vector.
"""
@inline get_eev_set(epv::Elemental_pv{T,S,Z}) where {T,S,Z} = epv.eev_set
@inline get_eev_set(epv::Elemental_pv{T,S,Z}, i::Int) where {T,S,Z} = epv.eev_set[i]

# docstring defined in M_abstract_part_struct.get_ee_struct
@inline get_ee_struct(epv::Elemental_pv{T,S,Z}) where {T,S,Z} = get_eev_set(epv)
@inline get_ee_struct(epv::Elemental_pv{T,S,Z}, i::Int) where {T,S,Z} = get_eev_set(epv, i)

"""
    eev_subset = get_eev_subset(epv::Elemental_pv{T,S,Z}, indices::Vector{Int}) where T
...
"""
@inline get_eev_subset(epv::Elemental_pv{T,S,Z}, indices::AbstractVector{Int}) where {T,S,Z} = epv.eev_set[indices]

"""
    eev_i_value = get_eev_value(epv::Elemental_pv{T,S,Z}, i::Int) where T
    eev_ij_value = get_eev_value(epv::Elemental_pv{T,S,Z}, i::Int, j::Int) where T
...
"""
@inline get_eev_value(epv::Elemental_pv{T,S,Z}, i::Int) where {T,S,Z} = get_vec(get_eev_set(epv, i))
@inline get_eev_value(epv::Elemental_pv{T,S,Z}, i::Int, j::Int) where {T,S,Z} =
  get_vec(get_eev_set(epv, i))[j]

"""
    set_eev!(epv::Elemental_pv{T,S,Z}, i::Int, vec::Vector{T}) where T
    set_eev!(epv::Elemental_pv{T,S,Z}, i::Int, j::Int, val:: T) where T

Set either the `i`-th elemental element-vector `epv` to `vec` or its `j`-th component to `val`.
"""
@inline set_eev!(epv::Elemental_pv{T,S,Z}, i::Int, j::Int, val::T) where {T,S,Z} =
  set_vec!(get_eev_set(epv, i), j, val)
@inline set_eev!(epv::Elemental_pv{T,S,Z}, i::Int, vec::Vector{T}) where {T,S,Z} =
  set_vec!(get_eev_set(epv, i), vec)

@inline (==)(ep1::Elemental_pv{T,S,Z}, ep2::Elemental_pv{T,S,Z}) where {T,S,Z} =
  (get_N(ep1) == get_N(ep2)) && (get_n(ep1) == get_n(ep2)) && (get_eev_set(ep1) == get_eev_set(ep2)) && (ep1.gpu == ep2.gpu)
@inline similar(epv::Elemental_pv{T,S,Z}) where {T,S,Z} =
  Elemental_pv{T,S,Z}(get_N(epv), get_n(epv), similar.(get_eev_set(epv)), S(undef, get_n(epv)); epv.gpu)
@inline copy(epv::Elemental_pv{T,S,Z}) where {T,S,Z} =
  Elemental_pv{T,S,Z}(get_N(epv), get_n(epv), copy.(get_eev_set(epv)), S(get_v(epv)); epv.gpu)

"""
    build_v!(epv::Elemental_pv{T,S,Z}) where T

Build the vector `epv.v` by accumulating the contribution of each elemental element-vector.
"""
function M_part_v.build_v!(epv::Elemental_pv{T,S,Z}) where {T,S,Z}
  reset_v!(epv)
  N = get_N(epv)
  for i = 1:N
...
"""
    minus_epv!(epv::Elemental_pv{T,S,Z}) where T <: Number

Build in place the `-epv`, by inversing the value of each elemental element-vector.
"""
minus_epv!(epv::Elemental_pv{T,S,Z}) where {T <: Number,S,Z} =
...
"""
    add_epv!(epv1::Elemental_pv{T,S,Z}, epv2::Elemental_pv{T,S,Z})

Build in place of `epv2` the elementwise addition of `epv1` and `epv2`.
"""
function add_epv!(epv1::Elemental_pv{T,S,Z}, epv2::Elemental_pv{T,S,Z}) where {T <: Number,S,Z}
...
function (+)(epv1::Elemental_pv{T,S,Z}, epv2::Elemental_pv{T,S,Z}) where {T,S,Z}
  _epv = copy(epv1)::Elemental_pv{T,S,Z}
  add_epv!(epv2, _epv)
  return _epv
end

(*)(val::Y, epv::Elemental_pv{T,S,Z}) where {T,Y,S,Z} = (*)(epv, val)

function (*)(epv::Elemental_pv{T,S,Z}, val::Y) where {T,Y,S,Z}
  _epv = copy(epv)::Elemental_pv{T,S,Z}
  N = get_N(_epv)
  for i = 1:N
...
function create_epv(
  ...
  v = (gpu ? CuVector{T} : Vector{T})(zeros(T, n))
  return Elemental_pv{T,S,Z}(N, n, eev_set, v)
end

function create_epv(vec_elt_var::Vector{Vector{Int}}, n::Int; type = Float64, gpu=false)
  eev_set =
    map((elt_var -> create_eev(elt_var; type, gpu)), vec_elt_var) #::Vector{Elemental_elt_vec{type,S,Z}}
  epv = create_epv(eev_set; n = n, gpu)
  return epv
end

"""
    set_epv!(epv::Elemental_pv{T,S,Z}, vec_value_eev::Vector{Vector{T}}) where T <: Number

Set the values of the elemental element-vectors of `epv` with the components of `vec_value_eev`.
"""
function set_epv!(epv::Elemental_pv{T,S,Z}, vec_value_eev::Vector{Vector{T}}) where {T <: Number,S,Z}
  N = get_N(epv)
...
"""
    v = scale_epv(epv::Elemental_pv{T,S,Z}, scalars::Vector{T}) where T <: Number
...
"""
function scale_epv(epv::Elemental_pv{T,S,Z}, scalars::AbstractVector{T}) where {T <: Number,S,Z}
  _tmp_v = get_v(epv)
...
"""
    v = scale_epv!(epv::Elemental_pv{T,S,Z}, scalars::Vector{T}) where T

Return a vector `v` from `epv` where the contribution of each element-vector is multiply by the corresponding value from `scalars`.
`v` is extract from `epv.v`
"""
function scale_epv!(epv::Elemental_pv{T,S,Z}, scalars::AbstractVector{T}) where {T,S,Z}
...
"""
function rand_epv(N::Int, n::Int; nie = 3, T = Float64, gpu=false)
  eev_set = [new_eev(nie; T = T, n = n, gpu) for i = 1:N]
  v = (gpu ? CuVector{T} : Vector{T})(zeros(T, n))
  S = typeof(v)
  Z = (gpu ? CuVector{Int} : Vector{Int})
  return Elemental_pv{T,S,Z}(N, n, eev_set, v; gpu)
end
...
"""
function ones_kchained_epv(N::Int, k::Int; T = Float64, gpu=false)
  n = N + k
  nᵢ = k
  eev_set = [ones_eev(nᵢ; T = T, n = n, gpu) for i = 1:N]
  v = (gpu ? CuVector{T} : Vector{T})(zeros(T, n))
  S = typeof(v)
  Z = (gpu ? CuVector{Int} : Vector{Int})
  return Elemental_pv{T,S,Z}(N, n, eev_set, v; gpu)
...
"""
function part_vec(; n::Int = 9, T = Float64, nie::Int = 5, overlapping::Int = 1, mul::Float64 = 1.0, gpu=false)
  ...
  eev_set = map(i -> specific_ones_eev(nie, i; T = T, mul = mul, gpu), indices)
  N = length(eev_set)
  v = (gpu ? CuVector{T} : Vector{T})(undef, n)
  S = typeof(v)
  Z = (gpu ? CuVector{Int} : Vector{Int})
  epv = Elemental_pv{T,S,Z}(N, n, eev_set, v; gpu)
  return epv
end

function Base.Vector(epv::Elemental_pv{T,S,Z}) where {T,S,Z}
  build_v!(epv)
  return get_v(epv)
end

"""
    epv = epv_from_v(x::Vector{T}, shape_epv::Elemental_pv{T,S,Z}) where T
...
"""
function epv_from_v(x::Vector{T}, shape_epv::Elemental_pv{T,S,Z}) where {T,S,Z}
...
"""
    epv_from_v!(epv_x::Elemental_pv{T,S,Z}, x::Vector{T}) where T

Set the values of the element partitioned-vector `epv` to `x`.
Usefull to define Uᵢ x, ∀ i ∈ {1,...,N}.
"""
function epv_from_v!(epv_x::Elemental_pv{T,S,Z}, x::Vector{T}) where {T,S,Z}
...
"""
    epv_from_epv!(epv1::Elemental_pv{T,S,Z}, epv2::Elemental_pv{T,S,Z}) where T

Set the elemental partitioned-vector `epv1` to `epv2`.
"""
function epv_from_epv!(epv1::Elemental_pv{T,S,Z}, epv2::Elemental_pv{T,S,Z}) where {T,S,Z}
...
"""
    (acc, res) = prod_part_vectors(epv1::Elemental_pv{T,S,Z}, epv2::Elemental_pv{T,S,Z}) where T
...
"""
function prod_part_vectors(epv1::Elemental_pv{T,S,Z}, epv2::Elemental_pv{T,S,Z}) where {T,S,Z}
  full_check_epv_epm(epv1, epv2) ||
    @error("different partitioned structures between eplo_B and epv_y")
  N = get_N(epv1)
  acc = (T)(0)
