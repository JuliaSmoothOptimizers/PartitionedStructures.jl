...
Performs the PBFGS update onto a copy of the elemental partitioned matrix `epm_B`, given the step `s` and the difference of elemental partitioned gradients `epv_y`.
Returns the updated copy of `epm_B`.
"""
...
Performs the PBFGS update onto the elemental partitioned matrix `epm_B`, given the step `s` and the difference of elemental partitioned gradients `epv_y`.
"""
function PBFGS_update!(epm_B :: Elemental_pm{T}, epv_y :: Elemental_pv{T}, s :: Vector{T}; kwargs...) where T
...
Performs the PBFGS update onto the partitioned matrix `epm_B`, given the element steps `epv_s` and the difference of elemental partitioned gradients `epv_y`.
"""
...
Performs the PSR1 update onto a copy of the elemental partitioned matrix `epm_B`, given the step `s` and the difference of elemental partitioned gradients `epv_y`.
Returns the updated copy of `epm_B`.
"""
...
Performs the PSR1 update onto the elemental partitioned matrix `epm_B`, given the step `s` and the difference of elemental partitioned gradients `epv_y`.
"""
function PSR1_update!(epm_B :: Elemental_pm{T}, epv_y :: Elemental_pv{T}, s :: Vector{T}; kwargs...) where T
  epv_s = epv_from_v(s, epv_y)
...
Performs the PSR1 update onto the elemental partitioned matrix `epm_B`, given the element steps `epv_s` and the difference of elemental partitioned gradients `epv_y`.
"""
...
Performs the PSE update onto a copy of the elemental partitioned matrix `epm_B`, given the step `s` and the difference of elemental partitioned gradients `epv_y`.
Returns the updated copy of `epm_B`.
"""
...
Performs the PSE update onto the elemental partitioned matrix `epm_B`, given the step `s` and the difference of elemental partitioned gradients `epv_y`.
"""
function PSE_update!(epm_B :: Elemental_pm{T}, epv_y :: Elemental_pv{T}, s :: Vector{T}; kwargs...) where T
  epv_s = epv_from_v(s, epv_y)
...
Performs the PSE update onto the elemental partitioned matrix `epm_B`, given the element steps `epv_s` and the difference of elemental partitioned gradients `epv_y`.
"""
