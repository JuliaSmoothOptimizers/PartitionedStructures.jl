...
Update the elemental partitioned-matrix `epm` with a partitioned quasi-Newton update considering the difference of elemental partitioned-gradients `epv_y` and the step `s` (or elemental steps `epv_s`).
