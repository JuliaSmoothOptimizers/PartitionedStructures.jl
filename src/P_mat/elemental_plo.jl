...
@inline set_eelo_set!(eplo::Elemental_plo{T}, i::Int, eelo::Y) where Y<:LOEltMat{T} where T = @inbounds eplo.eelo_set[i] = eelo
...
@inline (==)(eplo1::Elemental_plo{T}, eplo2::Elemental_plo{T}) where T = (get_N(eplo1)==get_N(eplo2)) && (get_n(eplo1)==get_n(eplo2)) && (get_eelo_set(eplo1)==get_eelo_set(eplo2)) && (get_permutation(eplo1)==get_permutation(eplo2))
@inline copy(eplo::Elemental_plo{T}) where T = Elemental_plo{T}(copy(get_N(eplo)),copy(get_n(eplo)),copy.(get_eelo_set(eplo)),copy(get_spm(eplo)), copy(get_L(eplo)),copy(get_component_list(eplo)),copy(get_permutation(eplo)))
@inline similar(eplo::Elemental_plo{T}) where T = Elemental_plo{T}(copy(get_N(eplo)),copy(get_n(eplo)),similar.(get_eelo_set(eplo)),similar(get_spm(eplo)), similar(get_L(eplo)),copy(get_component_list(eplo)),copy(get_permutation(eplo)))
...
"""
identity_eplo_LOSE(element_variables::Vector{Vector{Int}}; N::Int=length(element_variables), n::Int=max_indices(element_variables), T=Float64) = identity_eplo_LOSE(element_variables, N, n; T)
...
function PLBFGSR1_eplo(;n::Int=9, T=Float64, nie::Int=5, overlapping::Int=1, prob=0.5)
  ...
  eelo_set = map(i -> rand() > prob ? LBFGS_eelo(nie;T=T,index=i) : LSR1_eelo(nie;T=T,index=i), indices)
  N = length(indices)
  spm = spzeros(T,n,n)
  L = spzeros(T,n,n)
  component_list = map(i -> Vector{Int}(undef,0), [1:n;])
...
function PLBFGSR1_eplo_rand(N::Int, n ::Int; T=Float64, nie::Int=5, prob=0.5)
  eelo_set = map(i -> rand() > prob ? LBFGS_eelo_rand(nie;T=T,n=n) : LSR1_eelo_rand(nie;T=T,n=n), [1:N;])
  spm = spzeros(T,n,n)
  L = spzeros(T,n,n)
  component_list = map(i -> Vector{Int}(undef,0), [1:n;])
  no_perm = [1:n;]
