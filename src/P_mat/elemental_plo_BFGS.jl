...
@inline (==)(eplo1::Elemental_plo_bfgs{T}, eplo2::Elemental_plo_bfgs{T}) where T = (get_N(eplo1)==get_N(eplo2)) && (get_n(eplo1)==get_n(eplo2)) && (get_eelo_set(eplo1)==get_eelo_set(eplo2)) && (get_permutation(eplo1)==get_permutation(eplo2))
@inline copy(eplo::Elemental_plo_bfgs{T}) where T = Elemental_plo_bfgs{T}(copy(get_N(eplo)), copy(get_n(eplo)), copy.(get_eelo_set(eplo)), copy(get_spm(eplo)), copy(get_L(eplo)), copy(get_component_list(eplo)), copy(get_permutation(eplo)))
@inline similar(eplo::Elemental_plo_bfgs{T}) where T = Elemental_plo_bfgs{T}(copy(get_N(eplo)), copy(get_n(eplo)), similar.(get_eelo_set(eplo)), similar(get_spm(eplo)), similar(get_L(eplo)), copy(get_component_list(eplo)), copy(get_permutation(eplo)))
...
function PLBFGS_eplo(; n::Int=9, T=Float64, nie::Int=5, overlapping::Int=1)
  ...
  eelo_set = map(i -> LBFGS_eelo(nie; T=T, index=i), indices)
  N = length(indices)
  spm = spzeros(T, n, n)
  L = spzeros(T, n, n)
  component_list = map(i -> Vector{Int}(undef, 0), [1:n;])
...
function PLBFGS_eplo_rand(N::Int, n::Int; T=Float64, nie::Int=5)
  eelo_set = map(i -> LBFGS_eelo_rand(nie; T=T, n=n), [1:N;])
  spm = spzeros(T, n, n)
  L = spzeros(T, n, n)
  component_list = map(i -> Vector{Int}(undef, 0), [1:n;])
  no_perm = [1:n;]
