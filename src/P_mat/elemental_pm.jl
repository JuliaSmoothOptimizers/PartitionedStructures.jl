module ModElemental_pm
  ...
  "Symmetric elemental partitioned matrix"
  ...
  Create a partitionned matrix of type `T`.
  The partitionned matrix possess `N` identity elemental element matrices of size `nie` with randoms positions.
  """
  ...
  Create a partitionned matrix of type `T`.
  The partitionned matrix possess `N` elemental element matrices `ones(nie, nie)` whose positions are randoms.
  ...
  Create a partitionned matrix of type `T`.
  The partitionned matrix possess `N+n` elemental element matrices.
  `N` elemental element matrices `ones(nie, nie)` whose positions are randoms.
  The remaining `n` elemental element matrices are of value `[1]` , they are all over the diagonal.
  ...
  Define a partitioned nᵢ bloc separable matrix.
  Each elemental element matrix is composed of `1` except the diagonal that have value `mul`.
  """
  ...
  """
      n_i_SPS(n)
  Define a elemental partitioned matrix of size `n`.
  The partitioned matrix is composed by approximatively `(n/nie)*2` elemental element matrices.
  The partitioned matrix is generally not singular.
  """
  ...
  Define a elemental partitioned matrix form with `nie` elemental element matrices.
  Each elemental element matrix overlap the previous one and the next one by `overlapping`.		
  """
