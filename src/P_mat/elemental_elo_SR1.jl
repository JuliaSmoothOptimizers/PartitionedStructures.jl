...
@inline (==)(eelo1::Elemental_elo_sr1{T}, eelo2::Elemental_elo_sr1{T}) where T = (get_nie(eelo1)==get_nie(eelo2)) && (get_indices(eelo1)==get_indices(eelo2)) && begin v=rand(get_nie(eelo1)); (get_Bie(eelo1)*v==get_Bie(eelo2)*v) end
@inline copy(eelo::Elemental_elo_sr1{T}) where T = Elemental_elo_sr1{T}(copy(get_nie(eelo)), copy(get_indices(eelo)), deepcopy(get_Bie(eelo)), copy(get_cem(eelo)))
@inline similar(eelo::Elemental_elo_sr1{T}) where T = Elemental_elo_sr1{T}(copy(get_nie(eelo)), copy(get_indices(eelo)), similar(get_Bie(eelo)), copy(get_cem(eelo)))
